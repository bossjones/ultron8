...
- [Ultron-8 web api | uvicorn-gunicorn](#ultron-8-web-api--uvicorn-gunicorn)
  ...
  - [Advanced usage](#advanced-usage)
    - [Environment variables](#environment-variables)
      ...
      - [`WORKERS_PER_CORE`](#workerspercore)
    ...
    - [Custom `/app/gunicorn-prestart.sh`](#custom-appgunicorn-prestartsh)
...
# Ultron-8 web api | uvicorn-gunicorn
...
### Custom `/app/gunicorn-prestart.sh`

If you need to run anything before starting the app, you can add a file `gunicorn-prestart.sh` to the directory `/app`. The image will automatically detect and run it before starting everything.

For example, if you want to add Alembic SQL migrations (with SQLALchemy), you could create a `./app/gunicorn-prestart.sh` file in your code directory (that will be copied by your `Dockerfile`) with:
...
If you need to run a Python script before starting the app, you could make the `/app/gunicorn-prestart.sh` file run your Python script, with something like:
...
The default program that is run is at `/start-gunicorn.sh`. It does everything described above.

There's also a version for development with live auto-reload at:

```bash
/start-reload-gunicorn.sh
```
...
The additional script `/start-reload-gunicorn.sh` runs Uvicorn alone (without Gunicorn) and in a single process.
...
```bash
docker run -d -p 80:80 -v $(pwd):/app myimage /start-reload-gunicorn.sh
```
...
* `/start-reload-gunicorn.sh`: adding something (like `/start-reload-gunicorn.sh`) at the end of the command, replaces the default "command" with this one. In this case, it replaces the default (`/start-gunicorn.sh`) with the development alternative `/start-reload-gunicorn.sh`.

#### Technical Details

As `/start-reload-gunicorn.sh` doesn't run with Gunicorn, any of the configurations you put in a `gunicorn_conf.py` file won't apply.
