"""This module provides the basic models used in ultron8."""
...
class UltronCore(object):
    """The base object for all objects that require a session.

    The :class:`UltronCore <UltronCore>` object provides some
    ...
    def __init__(self, json, session):
        ...
        """
        # Either or 'session' is an instance of a UltronCore sub-class or it
        ...
        try:
            ...
            raise IncompleteResponse(json, kerr)
    ...
    def _repr(self):
        return "<ultron8-core at 0x{0:x}>".format(id(self))
    ...
    def _instance_or_null(self, instance_class, json):
        if json is not None and not isinstance(json, dict):
            raise UnprocessableResponseBody(
    ...
    def _iter(
        self, count, url, cls, params=None, etag=None, headers=None, list_key=None,
    ):
        ...
        :rtype: :class:`UltronIterator <ultron8.structs.UltronIterator>`
        """
        # from .structs import UltronIterator
        from ultron8.u8client.structs import UltronIterator

        return UltronIterator(count, url, cls, self, params, etag, headers, list_key)
    ...
    def refresh(self, conditional=False):
        ...
        if json is not None:
            if self._refresh_to is None:
                ...
                return self._refresh_to(json, self)  # pylint: disable=not-callable
        return self
    ...
    def new_session(self):
        ...
        :returns:
            A brand new session
        :rtype:
            :class:`~ultron8.u8client.session.UltronSession`
        """
        return session.UltronSession()
