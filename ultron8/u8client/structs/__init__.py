...
class UltronIterator(models.UltronCore, abc_collections.Iterator):
    """The :class:`UltronIterator` class powers all of the iter_* methods."""
    ...
    def __init__(
        ...
        models.UltronCore.__init__(self, {}, session)
    ...
    def _repr(self):
        return "<UltronIterator [{0}, {1}]>".format(self.count, self.path)
    ...
    def __iter__(self):
        ...
        cls = self.cls
        if issubclass(self.cls, models.UltronCore):
            ...
            if isinstance(json, dict) and self.list_key is not None:
                try:
                    ...
                    raise UnprocessableResponseBody(
                        "GitHub's API returned a body that could not be" " handled",
                        json,
                    )
...
class SearchIterator(UltronIterator):

    """This is a special-cased class for returning iterable search results.

    It inherits from :class:`UltronIterator <github3.structs.UltronIterator>`.
    ...
    def __init__(self, count, url, cls, session, params=None, etag=None, headers=None):
