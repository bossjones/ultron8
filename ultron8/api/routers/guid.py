...
from ultron8.api.models.guid import GuidIn, GuidUpdate, GuidOut
from ultron8.api.controller_utils import validate_guid
from ultron8.api.cache import cache
...
async def retrieve_guid(guid: str):
    ...
    return GuidOut(id=results.id, expire=results.expire, name=results.name)
...
async def create_guid(data: GuidIn) -> Type[GuidOut]:
    ...
    out = GuidOut(id=guid, expire=validated["expire"], name=validated["name"])
...
async def update_guid(guid: str, data: GuidUpdate):
    ...
    # Build serialized response
    out = GuidOut(id=results.id, expire=results.expire, name=results.name)
